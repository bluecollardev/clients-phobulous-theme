...
jQuery(document).ready(function($) {
	...
	$( document ).on( 'click', '.scroll-to', function ( event ) {
        // An improvement... take parent scrolling into account and move away from jQuery
        var $elemOffsetTop = $(this).offset().top;
        //var $hashOffsetTop = $(this.hash).offset().top;
        var $hash = $(this).attr('href');
        var $hashOffsetTop = $($hash).offset().top;
        console.log('scroll from ' + $elemOffsetTop + ' to ' + $hashOffsetTop + ', a change of ' + ($hashOffsetTop - $elemOffsetTop));
		// Carve off 158 px to account for menu
        $('html').velocity("scroll", { offset: $hashOffsetTop - 158, duration: 1000, easing:'easeOutExpo', mobileHA: false});
		event.preventDefault();
	});
    ...
    var noodlesComp = document.querySelector('.firebrand-hero');
    /* QC Time no jQuery here */
    /* Custom Phobulous */
    var tick = 0;
    var deg = 0;
    ...
    /*let ticker = setInterval(function () {
        ...
        noodlesComp.style.transform = 'rotate(' + (deg / 5) + 'deg)';
